...
if st.session_state.employees:
    delete_name = st.selectbox("Select Employee to Delete", df["Name"])
    if st.button("Delete"):
        st.session_state.employees = [
            emp for emp in st.session_state.employees if emp["Name"] != delete_name
        ]
        st.success("Employee Deleted")
# UPDATE
st.subheader("✏ Update Employee")
if st.session_state.employees:
    update_name = st.selectbox("Select Employee to Update", df["Name"])
